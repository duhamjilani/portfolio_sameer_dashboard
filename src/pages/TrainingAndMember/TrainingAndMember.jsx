...
const TrainingAndMember = () => {
  const [training, setTraining] = useState([
    { id: uuidv4(), title: 'React Training', year: '2023', description: 'Comprehensive React.js training program.',showTitle:0 },
    { id: uuidv4(), title: 'JavaScript Bootcamp', year: '2022', description: 'Intensive JavaScript learning experience.',showTitle:0 },
    { id: uuidv4(), title: 'Frontend Membership', year: '2021', description: 'Exclusive membership for frontend developers.',showTitle:1 },
  ]);

  const [editMode, setEditMode] = useState(null);
  const [editedItem, setEditedItem] = useState({ title: '', year: '', description: '',showTitle:'' });
  ...
  const handleEdit = (item) => {
    setEditMode(item.id);
    setEditedItem({ title: item.title, year: item.year, description: item.description ,showTitle:item.showTitle});
  };
  ...
  return (
    <div id="training-container" className="training-container">
      ...
      <button onClick={handleCreate} className="create-btn">Create New Training/Membership</button>
